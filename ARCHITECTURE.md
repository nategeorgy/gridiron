...
Last updated: 2026-09-24 (player leaderboards rebuilt as one page: five preset tabs, Custom, Edit Columns)
...
| `components/player/GameLog.jsx` | UI | One row per game under position-specific group headers, after a shared lead block that includes each game's **weekly finish** ("WR3") among the position. Covers the **whole season, not only the games played**: `buildSeasonLog()` merges his stat lines onto his team's fixture list, so a week still to come is a row of dashes and the bye is a greyed row in its place. A played fixture with no stat line reads the same way, which is correct (inactive, or not on the roster yet). Its column lists are its own (`GAMELOG_GROUPS`), no longer the career table's: a week wants the usage detail that explains one result, a career wants volume. Entries may be `{ id, label }` where a registry short reads wrong on one game ("HVT", not "HVT/G"). Deliberately **not** driven by the stat-grid tabs above it: a game log is the receipts, and re-cutting its columns whenever someone glances at another stat group would move the one stable table on the page. |
