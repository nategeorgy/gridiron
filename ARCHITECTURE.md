...
Last updated: 2026-09-14 (2026 season rollover: scoreboard + Vegas week rule, scaled game floors, retired board redirects, hand-loaded in-season routes)
...
| `components/player/HeadToHead.jsx` | UI | Two players, metric names down the middle, values either side, and **the margin pill on the winner's side in his colour** — the eye finds who leads a row before reading either number. Direction from the registry, so fewest drops wins the drops row. |
