...
Last updated: 2026-09-20 (the Second Level rebrand, and the credit bar unstuck)
...
| `components/home/ScoreboardCard.jsx` | UI | The rail's scoreboard. Tabs are labelled by **week number** (“Week 18” / “Week 1”) because that is what a manager calls them; which weeks those are comes from `/games/scoreboard`. **One line per game: away and logo, then the date above the score or kickoff, then logo and home** — each score beside its own team, and the losing side's abbreviation and score dim together. It replaced a row with the score at the card's far edge and only the winning number bold, which made a result a left-right-left read. The middle column is a fixed width so logos line up down the list; a missing logo keeps an empty box. Spread and total (or "Final") sit beneath. Capped and scrolling — a full week is 16 games. |
| `components/home/StandoutsCard.jsx` | UI | ⭐ **The week's standouts, top of the page.** Hand-picked players (`WEEKLY_STANDOUTS`) and one week of usage in three groups — on the field (Snap%, Routes, Route%), usage (Targets, Tgt%, Air yds), efficiency (YPRR, TPRR, FP/RR) — each value with its **rank at the position as a `FinishChip`**, so the colour means what it means on the player page: how deep the reader's league starts that position. Scoped to the one week (`weeks=`) so it still describes Week 1 once Week 2 is played. Groups sit beside the player at `xl` and stack beneath him below it. |
...
| `constants/signals.js` | config | ⚠️ **The hand-picked Command Center players (M10)**: `WEEKLY_STANDOUTS` (a season, a week and the players the standouts card shows), the two signal cards' lists, and the featured head-to-head, all read against `SIGNALS_SEASON`. (The preseason `OPPORTUNITY_OUTLOOK` set lived here until the standouts replaced it; it is in git history.) The selections are **hardcoded on purpose, with a known expiry** — a later pass decides whether they should be driven by the M3 regression indices or by something new. Only the *selection* is fixed; every number the cards show is fetched live in the reader's own scoring, so nothing here goes stale the way a pasted stat line would. Keeps the note from the 2025 picks that made Matthew Stafford's row honest: by points-versus-expected he was +1.5 (dead even), and he was on the card for a signal that metric cannot see — 46 passing TDs at a 7.71% rate against a 4.62% league average. That row is the standing evidence that a computed version of this card still needs **rate over baseline**. |
