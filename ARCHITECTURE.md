...
Last updated: 2026-09-22 (response cache on the leaderboard, scatter and comparison)
...
| `components/ui/NavDropdown.jsx` | UI (base) | The **Insight ▾ / Schedule ▾** nav menus — open on hover (desktop) and click/tap (touch), keyboard/Escape accessible. Items come from `constants/boards.js`. Open/close behaviour is `hooks/useHoverMenu.js`; the panel's offset is **padding on a hoverable wrapper**, never a margin. |
...
| `constants/boards.js` | config | **The 17 boards** (3 Insight + 5 Fantasy + 8 NFL + the SOS tool) — each declares its columns (metric ids), default sort/position, whether it's a scoring (fantasy) board, and whether it's an `insight` board (rendered by `InsightView` against `/stats/intelligence`). Also holds `LEADERBOARD_MENU` (the mega menu's area columns), `INSIGHT_TOOLS`, `SCHEDULE_ITEMS`, and — **exported but deliberately absent from `NAV_GROUPS`** — `DRAFT_ITEMS` and `EXPLORE_ITEMS`, which are hidden for launch alongside `HIDDEN_SECTIONS` in `App.jsx`. The **VORP board was removed outright**, and every `vorp` / `expected_vorp` / `replacement_ppg` column with it. Drives the **two** nav dropdowns plus the Leaderboards mega menu. Adding a board here is all it takes. |
