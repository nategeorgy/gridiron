...
| `components/schedule/WeekRail.jsx` | UI | ⭐ **The week picker for both schedule surfaces.** A `<select>` can say "Week 12 · no lines" only once you have opened it; the rail says it for the whole season at once — how much of each week is **played**, and for weeks ahead how much the market has **priced**. Also exports `defaultWeek()`, the shared "which week do we open on" answer (the first unfinished week), which callers keep OUT of the URL. |
