# Second Level — Architecture & Project Map
...
Last updated: 2026-09-24 (player leaderboards rebuilt as one page; the player page takes the same five tabs)

---

## 1. The 30-second overview

Second Level is a **fantasy-first NFL analytics web app**. It is built as **three
...
| `Second-Level-Technical-Walkthrough.docx` | doc | A standalone Word write-up (not wired into the code). |
...
1. Browser loads the React app (`frontend/`). The router (`App.jsx`) shows
   `LeaderboardView` at `/leaderboards/fantasy` (the home route `/` is the Command
   Center; the old `/leaderboard` and the fourteen M12 board paths redirect here).
2. The page reads the user's chosen filters (tab, position group, season, scoring) from
   the URL and calls a **hook** (`useIntelligence`), which calls a **service**
   (`services/insight.js`), which uses the shared **axios client** (`services/api.js`)
   to make an HTTP GET to:
   `http://localhost:8000/api/v1/stats/intelligence?season=2025&positions=RB,WR,TE&metric=fantasy_points&scoring=ppr&percentiles=...`
3. The backend's **stats router** (`backend/app/routers/stats.py`) handles that URL.
   It parses the `scoring` string into a `ScoringConfig` (`app/scoring.py`) and asks
   the intelligence engine for the scored season, which is cached per data version
   (`app/cache.py`), so a repeat request reads almost nothing from disk.
4. The backend computes each player's fantasy points **live** from the raw components
   + the scoring config, filters to the requested positions, sorts, attaches a
   percentile within each player's own position, and returns JSON:
   `{ "data": [...players...], "total": 342, ... }`.
5. Back in the browser, React Query caches the JSON and `StatTable` renders it. Column
   labels/formats come from the **metric registry** fetched from `GET /api/v1/metrics`.
...
| `public/favicon.svg` | The browser-tab icon: the 2L mark on its dark tile. Drawn at 84% of the tile rather than the app icon's 72%, because the mark has interior detail that dies at 16px. |
| `public/apple-touch-icon.png` | 180x180 home-screen icon, exported from `brand/second-level/mark/2l-icon.svg`. |
| `public/icon-512.png` | 512x512 icon, for anything that wants a large raster. |
...
| `App.jsx` | routing | The route table. `/` → **Home** (Command Center); **`/leaderboards/:tab` → `LeaderboardView`**, one route for all six tabs so switching tabs keeps the page (and an open Edit Columns panel), with `/leaderboards` → Fantasy; `/insight/*` → `InsightView` and `SosView`; **`/schedule/*` → the M10 schedule surface** (`GamesView`, `ScheduleGridView`, and the Vegas board moved from `/insight/vegas`, which redirects); `/players/:playerId` → PlayerProfile; `/teams` → Teams. ⭐ **`HIDDEN_SECTIONS` (`draft`, `explore`) redirects those whole subtrees to `/` for launch** — the pages still live in `pages/` and still compile, but a bookmark, a saved view or a search result must not land on an unfinished page; un-hiding is this list plus `NAV_GROUPS`. **`LegacyBoard` redirects the fourteen M12 board paths, the six they had absorbed and `/leaderboard`** to the tab and position group that answer the same question (`LEGACY_BOARD_REDIRECTS` in `constants/leaderboards.js`), and `RETIRED_BOARDS` sends `/insight/vorp` → `/insight/opportunity`, **query string kept** in both (there is no catch-all, so an unmatched path renders nothing at all, header included); and **`/styleguide` → the design-token studio, registered only when `import.meta.env.DEV`** so it never ships. All wrapped in `Layout`. |
...
| `pages/Home.jsx` | page | ⭐ **The home screen (`/`) — the Command Center**, rebuilt in M10 as the "Fantasy Desk" layout and rearranged in September 2026. The visible heading reads **"Highlighted Data"**; "Command Center" is the page's name in the code and the docs, not on screen. It builds a **keyed `cards` object and takes its arrangement from `constants/homeLayout.js`**, so where a card sits is data rather than JSX. A card with nothing to show returns `null` and the layout skips it. Wide column: Trending Players, the watchlist, trending usage, Expected vs Actual, opportunity leaders, the featured head-to-head. Rail: the scoreboard, last week's scoring, quarterbacks. Composes `components/home/*` and owns only the data fetching. **Two seasons are in play from January to September** — the fantasy cards describe the last season *played*, the scoreboard describes the schedule, which runs a year ahead — so every card names its own season rather than the page claiming one. |
| `pages/GamesView.jsx` | page | ⭐ **The schedule as a slate (M10, redesigned Sept 2026).** Cards grouped by kickoff slot inside one week, by week across several — grouping 272 games into seven slot buckets would be meaningless, and grouping 16 by week would be one heading. Replaced a flat six-column table whose kickoff slot was invisible and whose "Implied A/H" was two numbers in a shared cell. |
| `pages/VegasView.jsx` | page | ⭐ **Game environment as a fantasy input (M6.4, redesigned Sept 2026).** One scroll, summary then detail: `TeamEnvironmentRail` then `OffenseCard`s, both ordered by implied total. The old Players/Games toggle is gone — *that* was the fix. Structure comes from `/games` (it carries logos and implied totals); `/stats/vegas?view=players` supplies the chips. |
| `pages/LeaderboardView.jsx` | page | ⭐ **The player leaderboards, one page (September 2026).** `/leaderboards/:tab`, where the tab is the question (Fantasy, Usage, Efficiency, Expected, Tracking, or Custom) and the **position group** (QB, RB, WR, TE, WR/TE, RB/WR/TE, All) is a control on the page that stays put across tabs. Tab, group and a custom `cols=` list all live in the URL, so saved views and shared links carry the whole board. Served by **`/stats/intelligence`** (the cached engine, and the only endpoint with TDs over expected). ⚠️ **A column change costs no request**: the percentile request names every stat the *group* has (`poolColumns`), not the columns on screen, and rows already carry every stat, so Edit Columns redraws the table in the browser. A custom board keeps the sort of the preset it came from, so the first edit does not re-sort the table. `cols` is read raw because absent ("no custom board yet") and empty ("the user cleared every column") mean different things. |
...
| `pages/PlayerProfile.jsx` | page | ⭐ **One player, rebuilt in M13.** The reading order is the order the questions get asked: identity → this season's stats → the career and its finishes → how that ranks → a head-to-head → the game log. Composes `components/player/*` and owns only the fetching and the season choice. **Everything is position-driven** from `constants/playerPage.js`, so a quarterback's page is the same components over a different column list. **One request serves the season**: the stat grid, the radar and the percentile panel are three views of a single `/stats/intelligence` row, which is used rather than the leaderboard because it carries VORP and the other query-time columns *and* a percentile for every metric — and adding the comparison player to that same call is what keeps both halves of a matchup on identical pools. Takes an optional **`playerId` prop** (M9) so the draft room can render it inside a dialog; the back link and the Compare section hide when embedded (inside the draft room the board behind the modal already *is* the comparison). The header no longer shows the dated depth-chart badge (removed in M13); `/players/{id}` still returns `depth_chart`, and the team page still dates its own depth chart. |
| **`components/player/`** | UI | ⭐ **The M13 player-page pieces**, split out for the same reason as `components/home/`: seven surfaces in one file is how seven treatments of the same table appear. |
| `components/player/CareerTable.jsx` | UI | Season-by-season, with each season's **finish among the position** — "WR4" is what a manager remembers a season as, and it is the column that makes a career readable at a glance. Re-ranked in the caller's scoring by the API. Clicking a season drives the rest of the page, so the table doubles as the season picker. After the fixed lead block (season, team, games, points, PPG, finish, snap share) come **the table's own view**: one of the leaderboard's five tabs, or a custom list, chosen with a `TableViewBar` in its header (default Fantasy). The finish chip carries the *per-game* finish in its tooltip, which is a second pool: total points ranks everyone who played, PPG ranks only players who qualified. |
| `components/player/SeasonStatGrid.jsx` | UI | The season in one section. **A headline** of exactly ten stats (per position, `HEADLINE_STATS`) as one open row — no tile per stat, no rules — each carrying its **positional rank** ("WR4") tinted by its percentile. Ten across from `xl`, five below, two on a phone, and never a horizontal scroll. **Beneath it, the leaderboard's five tabs (Fantasy, Usage, Efficiency, Expected, Tracking) as five rows** in a single scroll frame, so one scrollbar moves all five; they are `seasonBoards()`, the leaderboard presets for the position, with FPPG kept to the Fantasy row. ⚠️ The board names are a static column **outside** the scroller, not a sticky one inside it: a sticky label needs a background to mask the cells sliding under it, and a label nothing can scroll under can be plain text. Row height is fixed so the two columns stay in step, and each cell labels itself because the five boards cannot share a header row. |
| `components/player/SeasonRadar.jsx` | UI | Ten to twelve percentiles as wedges, grouped and coloured by what they measure (Opportunity / Production / Advanced, per position in `RADAR_GROUPS` — a quarterback's chart has no Opportunity group). A group's colour follows the group, not its place in the list. **Wedge length is the percentile and the faint full-length track behind it is 100**, so what is *missing* is as legible as what is there — the reason this reads faster than the same ten numbers listed. |
| `components/player/PercentileLadder.jsx` | UI | Every headline metric ranked within the position, as a **bullet**: quartile context bands behind a thin measure line, so the row says where average is without parking a marker on top of the number. Keeps "not ranked" (the value is real, the rank would not be — a receiver's rushing yards) visibly distinct from "not recorded" (M8). |
| `components/player/HeadToHead.jsx` | UI | Two players, metric names down the middle, values either side, and **the margin pill on the winner's side in his colour** — the eye finds who leads a row before reading either number. Direction from the registry, so fewest drops wins the drops row. The rows are `MarginTable`. |
| `components/player/MarginTable.jsx` | UI | **The head-to-head table itself**, shared by the player page and the Command Center's Head to Head card so one matchup reads identically on both. Each row arrives with its pair of values already resolved, which is what lets the two callers keep their own response shapes. |
| `components/player/CompareRadar.jsx` | UI | Deliberately the **same chart as `components/home/HeadToHeadCard`** — nested plates, two polygons, real values badged on each axis — so a matchup reads identically on the home page and here. Shape is each player's percentile *within his position*, not against the other player. |
| `components/player/ComparePicker.jsx` | UI | Picks the comparison player, **restricted to the subject's own position**: every axis beside it is a percentile within a position pool, and a receiver's target share against a quarterback's is two questions sharing an axis. (The M4 `/explore/compare` builder allows mixed positions and handles it by intersecting metrics; this is the *matchup* view instead.) |
| `components/player/GameLog.jsx` | UI | One row per game under position-specific group headers, after a shared lead block that includes each game's **weekly finish** ("WR3") among the position. Covers the **whole season, not only the games played**: `buildSeasonLog()` merges his stat lines onto his team's fixture list, so a week still to come is a row of dashes and the bye is a greyed row in its place. A played fixture with no stat line reads the same way, which is correct (inactive, or not on the roster yet). After the lead block come **the table's own view**: one of the leaderboard's five tabs or a custom list, chosen with its own `TableViewBar` (default Usage), so the game log can show Usage while the career shows Fantasy. `gameValue()` derives the per-game rates, sums and actual-minus-expected differences no stored column holds, and `GAMELOG_LABELS` relabels where a registry short reads wrong on one game ("HVT", not "HVT/G"). Deliberately **not** driven by anything else on the page: a game log is the receipts, and it changes columns only when someone picks one of *its* tabs. |
| `components/player/TableViewBar.jsx` | UI | A player-page table's view controls: the leaderboard's tabs (`LeaderboardTabs` at `size="sm"`, with `onSelect` since the tab is a query param, not a path) plus **Edit Columns**, which opens the leaderboard's `ColumnEditor` with that table's lead-block stats excluded and "In this table" as its heading. |
| `components/player/FinishChip.jsx` | UI | A positional finish ("WR4") coloured by **how deep the user's league starts that position**, shared by the career table and the game log so a WR9 season and a WR9 week are the same colour. `startingDepth` mirrors the backend's `replacement_ranks` (`app/league.py`) **without the bench** — dedicated slots (superflex as QB) plus a proportional flex share, rounded half-to-even like Python — giving five steps per position — dark green (top), light green (starter), yellow (flex), light red (up to one league's worth of ranks past the starting line), dark red (deeper). In a default 12-team league QB22 is light red while WR22 is an ordinary start. The tint values were chosen by search over both themes (neighbouring steps ≥10.6 apart in OKLab, all text ≥4.8:1). Deliberately not a percentile of the pool — a weekly pool holds ~150 receivers, so a percentile would paint WR40 as upper-half. A tint with coloured text rather than a solid fill, measured at ≥5.3:1 in both themes. |
| `components/player/sides.js`, `percentile.js`, `metricTip.js` | UI | The three things more than one of the above needs: the blue/gold identity pair (shared with the home head-to-head card, gold badge taking dark ink for contrast), the diverging percentile tint, and the tooltip payload — which passes `hint: null`, since nothing on this page sorts. |
...
| `components/Layout.jsx` | UI | The app shell: frosted sticky header with brand, nav (Home, **Insight ▾**, **Schedule ▾**, **Leaderboards ▾**, Teams, all from `NAV_GROUPS`), search box, the theme toggle, and **the account menu**; renders the current page inside. `WIDE_ROUTES` (routes opting out of the 1280px cap) is **empty** while the draft room — its only member — is hidden for launch. The page background (the Liquid Glass "environment") is painted on `<body>`. The brand is the stacked `BrandLockup`, wrapped in a link to `/`. The shell is a **flex column** with `<main>` on `flex-1` and the footer on `mt-auto`, so the footer sits at the bottom of a short page without being fixed and `<main>` reserves nothing. ⚠️ **The home page owns its own width**: its arrangement decides how much room the columns need, so `<main>` drops its cap on `/` and Home centres its own container. Every other route keeps the 1280px shell. ⚠️ **`contentWidth()` is the single owner of the content column's width**, as a number rather than a Tailwind class, and both `<main>` and the credit bar read it. The home page names its own (`HOME_LAYOUT.container`); everything else takes the 1,280px shell. |
| `components/Brand.jsx` | UI | ⭐ **The Second Level identity.** `BrandMark` (the 2L, the only inline SVG here because it is artwork), `BrandLockup` (mark + SECOND LEVEL, used in the header) and `SlashLockup` (SECOND / LEVEL over the tagline). Everything except the 2L is **live text**, not outlines: the app already loads Inter 400-700, so the wordmarks cost nothing in the bundle and stay selectable. The outlined SVGs in `brand/second-level/` exist for the places a webfont cannot be relied on (social cards, PDFs, email). Colour follows the theme: the 2's half is `currentColor` so it inverts on light, and the green is `var(--accent)` (the deeper `#00b06a` on light, where the brand `#11c759` would fail contrast). |
| `components/Footer.jsx` | UI | ⭐ **The site-wide credit bar**, carrying the `SlashLockup`, every upstream the pipeline reads (nflverse, Next Gen Stats, Pro Football Reference, FantasyPros, ESPN for images), the statement that NFL team names and logos belong to the NFL and its teams, the affiliation disclaimer, and `CONTACT_EMAIL`. **A `glass-card` in normal flow**, inside the same max-width wrapper `<main>` uses, so its edges line up with the content above it rather than running to the viewport; it takes `wide` for the same reason `<main>` does. It used to be `position: fixed` from `sm` up on a bespoke `.glass-footer` surface, which spent a permanent strip of every screen (37px at 1280+, 57px between 640 and 1280, **157px at 390px**) and forced `<main>` to reserve that height with matching `pb-*` steps. `.glass-footer` is now **deleted**: its `backdrop-filter` blurred nothing once the bar stopped floating over scrolling content, and the card material is shared. Links are `--fg` with a `--muted` underline, never `--accent` (2.44:1 on the light surface). ⚠️ **The width is passed in, not chosen here.** It used to pick between two Tailwind classes of its own, and the moment a route wanted a third width (the home page, at 1,560px) the credit bar was visibly narrower than the content above it. `Layout` owns the number for both. |
...
| `components/home/ScoreboardCard.jsx` | UI | The rail's scoreboard. Tabs are labelled by **week number** (“Week 18” / “Week 1”) because that is what a manager calls them; which weeks those are comes from `/games/scoreboard`. **One line per game: away and logo, then the date above the score or kickoff, then logo and home** — each score beside its own team, and the losing side's abbreviation and score dim together. It replaced a row with the score at the card's far edge and only the winning number bold, which made a result a left-right-left read. The middle column is a fixed width so logos line up down the list; a missing logo keeps an empty box. Spread and total (or "Final") sit beneath. Capped and scrolling — a full week is 16 games. Each side carries the team's **season record** (from the window's `records` map) on the far side of its abbreviation, so the abbreviation stays next to its own logo and the two halves of the row stay mirror images. |
| `components/home/TrendingPlayersCard.jsx` | UI | ⭐ **Trending Players, top of the page** (renamed from Week Standouts, September 2026). Hand-picked players (`TRENDING_PLAYERS`) with **four stats each and the change stated underneath rather than drawn** — an earlier build put a dumbbell in every cell, showing both the move and where the player sat league-wide, and the second job was already done by the rank beside his points. ⚠️ **Each pick names its own stats and its own basis** (`TRENDING_BASIS`): a receiver against his own last week, a back against the teammate he took the carries from, a quarterback against the position for the whole season. Stats are metric ids, so label, format and `higher_is_better` all come from the registry. Scoped to one week (`weeks=`) so it keeps describing that week once later ones are played. |
| `components/home/TrendingCard.jsx` | UI | The live trending board, rendered only once the endpoint has rows (about seven weeks into a season). Rows are **dumbbells** — an earlier draft drew "before" as a second stacked bar and it was invisible, since every row is a move in one direction. Paging is a bottom-centred chevron+dot control (absolutely-positioned link beside it, so the pager centres against the *card* rather than the space left over). Its preseason fallback — the hand-picked 2026 outlook — was removed when the standouts took the top slot. |
| `components/home/HeadToHeadCard.jsx` | UI | Radar by default, table behind a toggle. Radar axes are **percentiles**, so the *shape* is the comparison; each axis carries both real values with the leader's **badged in his own colour**, which is what removes the need for a legend. ⚠️ The badge inks are per-hue, not one inherited white: white on the gold measures **2.17:1** in the light theme. The table view is the player page's `MarginTable` — values either side, margin pill on the leader's side — which replaced a tug-of-war of percentile bars. Headshots are 96px: this is the one card whose subject is two specific players rather than a list, so the faces carry it. |
| `components/home/ExpectedActualCard.jsx` | UI | ⭐ **Expected vs Actual (September 2026)** — one plot in the wide column replacing the two rail cards (Underperformers / Regression Candidates), which drew the same quantity twice on two scales and never said what caused it. Every player is a faint dot from `/stats/scatter`, the diagonal is par, and the picks are **headshots on short leaders**, placed by a routine that tries eight angles at three distances and keeps clear of its neighbours — ⚠️ the reserved box is the photo **plus its two lines of text**, because reserving only the circle put one player's points-over-expected on another's face. Hovering one gives `explainGap()`: every expected component of the player's dominant phase that falls on the same side as his points, biggest gap first, **derived not written**, with direction respected so fewer interceptions than expected reads as a surplus. Deliberately **not red/green**: "below expected" is a *buy*. |
| `components/home/BoardCards.jsx` | UI | The four ranked tables — weekly scoring (with a FLEX tab, RB/WR/TE through `positions=`, and touchdowns in the stat line when there were any), opportunity leaders, quarterbacks, watchlist. All are a board endpoint with a different column set, which is the point: the cards a manager checks weekly should not be a second implementation of the boards they can open in full. **My Players** is the one served by `/stats/intelligence` rather than the leaderboard (FOR is query-time), and each of its seven values carries its **percentile at that position** beneath it, the same pairing the boards use; a position a metric is not ranked in renders a dash, so a quarterback's target share stays visibly unranked rather than looking like a zero. |
...
| `components/ui/NavDropdown.jsx` | UI (base) | The **Insight ▾ / Schedule ▾ / Leaderboards ▾** nav menus. They open on hover (desktop) and click/tap (touch), and are keyboard/Escape accessible. Items come from `constants/boards.js`. Open/close behaviour is `hooks/useHoverMenu.js`; the panel's offset is **padding on a hoverable wrapper**, never a margin. |
| `components/schedule/WeekRail.jsx` | UI | ⭐ **The week picker for both schedule surfaces.** A `<select>` can say "Week 12 · no lines" only once you have opened it; the rail says it for the whole season at once, as a meter under each week: how much of it is **played**, and for weeks ahead how much the market has **priced**. The state was also spelled out in words under the number ("final" / "live" / "priced" / "no line") until 2026-09-17, which was four characters of chrome per week saying what the meter and the hover text already said. Also exports `defaultWeek()`, the shared "which week do we open on" answer (the first unfinished week), which callers keep OUT of the URL. |
| `components/schedule/GameCard.jsx` | UI | One fixture as a card: both teams with logos, the score where it has been played and the spread where it has not (same slot — they answer the same question at different times), and the implied split beneath. |
| `components/schedule/ImpliedSplit.jsx` | UI | The implied-total bar. Splits a game's total by the spread so the scoring environment reads without arithmetic. ⚠️ Encodes the **split, not the size** — two games with the same spread and different totals draw the same bar, which is why the total is always printed between the figures. An unpriced game renders an empty track, never a 50/50, and since 2026-09-17 nothing else: the "no line yet" caption it used to carry is now the track's hover text, because in August that caption is most of the board. |
| `components/schedule/TeamEnvironmentRail.jsx` | UI | ⭐ **The Vegas board's summary half (direction F).** Every offence this week, one bar each, ranked by implied total. One row per *distinct value* — the fix for a board that used to rank ~300 players on 32 numbers. Sub-median bars are drawn faint rather than in a second hue, because `--accent` already means "more points" here. |
| `components/schedule/OffenseCard.jsx` | UI | ⭐ **The Vegas board's detail half (direction E).** The rail above, expanded: one offence, its share of the game total, and its fantasy-relevant players as chips. ⚠️ No text sits inside the bar — white on `--accent` measures 2.83:1 in the light theme. |
| `components/schedule/kickoff.js` | util | Date and kickoff formatting plus `slotOf()` / `SLOT_ORDER`, the named windows a week is actually read in. The list starts at **Wednesday Night**, because a week occasionally opens on one (two in 2026) and those games belong at the top of the slate rather than in "Other" at the bottom. Kickoff is a naive time that always means Eastern, so it is formatted with a literal "ET" and never timezone-converted; dates are split by hand rather than parsed as a local `Date`, which would shift the day west of UTC. |
| `components/StatTable.jsx` | UI | The ranked table behind every board. Renders **sections** (a spanning header row over named column groups, keyed by position because a custom board can repeat a name and FPPG's leading section has none), a **percentile** beneath each value tinted on a scale that *diverges around the median* (a single-direction ramp turns a 30-column row uniformly green and distinguishes nothing), a dash where a rank is deliberately withheld, and centre-aligned columns. Headers sort on click and carry `StatTooltip`. ⚠️ `signed` (positive/negative value tinting) is empty on the player leaderboards, where only percentiles carry colour; the Insight boards keep it, because there the sign *is* the finding. |
...
| `components/ScoringPill.jsx` | UI | ⭐ **The Command Center's scoring switcher**, as a single pill — "Scored in [PPR ▾]". It used to be a *link* to the leaderboard, which made the one global setting on the page the only thing you had to leave the page to change. A native `<select>` on purpose: four values, no search, and a custom menu would need portalling for the usual `backdrop-filter` reason. Options carry an explicit `--surface-solid` background or they inherit the translucent surface and go unreadable in dark. |
| `components/ScoringControl.jsx` | UI | The scoring picker: one dropdown over the four presets (PPR / Half / Standard / TE-Premium), emitting a scoring spec string. Takes a `label` (default "League Scoring"; the leaderboards say "Scoring") and `bare` to sit inside another filter row, which is where the leaderboards put it, beside Team. The **"Customize" per-stat weights panel and the league-profile bar beneath it were cut before launch** — what is left is a lens, switched as freely as a column sort, not a setting. |
...
| **`components/leaderboard/`** | UI | ⭐ **The leaderboard page's pieces (September 2026).** `LeaderboardTabs.jsx`: the five presets plus Custom as links carrying the query string, so the group and filters survive a switch; Tracking is disabled for All; the Custom tab with no custom board yet starts one from what is on screen and opens the editor. `PositionGroupPicker.jsx`: the seven fixed groups as one segmented control (a group narrows the table, never the percentile pool). `ColumnEditor.jsx`: the **Edit Columns slide-out**, portalled to `document.body`, chosen from four mocked options. **On your board** (top) is the table's columns in order under the table's own section headers, dragged with pointer events (mouse, pen or finger) or moved with the arrow keys, with an × on each, plus Clear and Reset; a drag is previewed locally and committed once on release. **Add stats** (below) is the group's whole pool as chips, split by the page's five tabs with a count of what is on the board from each; search looks across every tab. A line at the bottom says what the last stat hovered or focused measures. Ordering and picking share one view, so there is no Reorder mode. It edits **live**, which is why it is a slide-out and not a modal: the board is the preview. In a mixed group, a stat only some positions have is dimmed and says who has it. |
...
| `components/PositionFilter.jsx` | UI | ⭐ **Multi-select position filter (M12)**, now used by the Insight boards and the mock draft (the leaderboards use `PositionGroupPicker`). A single-choice dropdown could not answer "receivers and tight ends", which is what a flex decision asks. Each position lit in its own `--position-*` colour. ⚠️ Narrows the board, never the pool: filtered to WR+TE, two players with identical receiving yards read different percentiles because each is ranked among his own position. |
| `components/TimeframeFilter.jsx` | UI | ⭐ **The weeks picker (M12).** One control: an explicit set of weeks, or none for the full season. **Last 4 / Last 8** resolve to the last weeks *actually played*, read from `/games/weeks` — the one thing a bare week grid cannot express, since mid-season the reader would otherwise need to know what week it is. The button names the weeks rather than counting them ("Week 2", "Weeks 1, 2", with runs of three or more as "Weeks 1–4"), via the exported `formatWeeks()`. Portalled, for the `backdrop-filter` stacking reason. |
...
| `hooks/usePlayer.js` | data | Fetches a player profile, game log, **target-depth buckets** (`usePlayerTargetDepth`), the **season-by-season career** with its finishes (`usePlayerCareer`, M13 — scoring-aware, so the spec is in the query key) and **one or two players' season rows with percentiles** (`usePlayerSeason`, M13 — served by `/stats/intelligence`, which is what makes VORP and a matchup's shared pools possible in a single request). |
...
| `hooks/useHoverMenu.js` | state | ⭐ **Open/close behaviour for the header hover menus** (`NavDropdown`; the Leaderboards mega menu that shared it was retired in September 2026). Fixes two bugs of geometry rather than state: no grace period (leaving the trigger closed the menu on the same frame, so the diagonal travel toward an item lost it) and a dead 6px gap belonging to neither element. A close is deferred ~260ms and cancelled on re-entry; callers carry the offset as padding on a hoverable wrapper. A module-level registry closes any sibling the moment another opens, so the delay never leaves two panels up. |
...
| `services/players.js` | network | Player profile, game log, search, **target-depth**, and the **career** call (M13). |
...
| `constants/boards.js` | config | **The Insight boards and nav.** The three Insight boards (rendered by `InsightView` against `/stats/intelligence`), `INSIGHT_TOOLS`, `SCHEDULE_ITEMS`, and `NAV_GROUPS` (Insight, Schedule, Leaderboards), plus, **exported but deliberately absent from `NAV_GROUPS`**, `DRAFT_ITEMS` and `EXPLORE_ITEMS`, which are hidden for launch alongside `HIDDEN_SECTIONS` in `App.jsx`. The player boards moved out to `constants/leaderboards.js`. |
| `constants/playerPage.js` | config | **What a position's player page shows** (M13): the ten-stat headline, the radar's slices, the percentile panel's groups, the head-to-head rows and the compare axes, keyed by position and read through `forPosition` (WR fallback). Since September 2026 the season grid, career table and game log read the **leaderboard presets** instead: `seasonBoards()` for the season rows, `TABLE_VIEWS` for each table's URL key, default tab, the lead-block stats it leaves out and **the player page's own Fantasy tab** (the position's box score plus YPC, or Y/TGT and Y/REC, where the leaderboard's shows every phase), and `gameValue()` for per-game values. `SIGNED_COLUMNS` is the one list of stats shown with an explicit "+". |
| `constants/leaderboards.js` | config | ⭐ **The player leaderboards (September 2026).** `LEADERBOARD_TABS` (the five presets, which are also the nav menu), `CUSTOM_TAB`, `POSITION_GROUPS` (default **RB/WR/TE**, kept out of the URL; All is an explicit `all`), and a **preset per position group per tab**: every stat has one home tab (only targets, carries and attempts repeat, as box score on Fantasy) and **FPPG leads every preset** under an unnamed header. WR/TE is the tight-end layout; RB/WR/TE is curated for flex; All has no Tracking. `groupPool()` merges the single-position presets into what Edit Columns offers and what a custom `cols=` is read against; `customSections()` keeps the user's order and gives neighbouring same-tab columns one header. Also `LEGACY_BOARD_REDIRECTS`. Next Gen's own CPOE, catch rate, intended air yards and air-yard share are deliberately absent. |
...
| `constants/storage.js` | config | The `localStorage` key for scoring, plus safe read/write helpers. |
| `constants/homeLayout.js` | config | ⭐ **How the Command Center arranges its cards** — container width (1,560px), column templates, which cards sit in which column, and which column is sticky. Chosen by measuring rather than by eye: every card's height was taken at nine column widths, four arrangements were built from those numbers, and this one won. Three constraints live here. ⚠️ **Expected vs Actual grows with its column** (a fixed-ratio SVG at 100% width: 457px tall at 660, 1,029px at 1,500), so it is the tallest card in the wide column and takes any extra room given to it. ⚠️ **Two ratios, and the wide one is measured, not Tailwind's**: 1.75/1 only clears the rail's widest table (Quarterbacks, 470px plus 32px of card padding) from a 1,440px viewport, so the rail takes a bigger share (1.28/1) below that. ⚠️ **Nothing is sticky**: the rail now runs ~1,600px, and `position: sticky` on a column one and a half viewports tall does nothing — nor would pinning the scoreboard alone, since a sticky grid item only travels within its own row. |
| `constants/signals.js` | config | ⚠️ **The hand-picked Command Center players (M10)**: `TRENDING_PLAYERS` (a season, a week, and per pick its four metric ids plus its `TRENDING_BASIS`), `EXPECTED_VS_ACTUAL` (the plot's picks, **not split by side** — which side a player is on is read from his own points over expected, so a pick that crosses par reclassifies itself instead of sitting under a heading that has stopped being true), and the featured head-to-head, all read against `SIGNALS_SEASON`. The selections are **hardcoded on purpose, with a known expiry** — a later pass decides whether they should be driven by the M3 regression indices or by something new. Only the *selection* is fixed; every number the cards show is fetched live in the reader's own scoring, so nothing here goes stale the way a pasted stat line would. |
...
| `cache.py` | ⭐ **The engine cache.** Added after the Insight engine exhausted Supabase's Disk IO budget (September 2026): nothing the engine computes is stored, so every board, player page and draft page rebuilt the same pools from a table that does not fit in the free instance's memory. `VersionedCache` is a bounded in-process LRU keyed on the arguments **and a data version** — the insert/update/delete counters Postgres keeps for `player_stats`, `players`, `teams` and `games` — so any write (the pipeline, a migration, a hand-loaded routes file) forces a miss. No TTL to tune (a one-hour backstop only) and no ingest-side hook. Used by `intelligence.py` (week bounds, career totals, scored windows), `seasons.py` and `sos.py`. Also holds **`cached_response`**, a decorator that caches a whole endpoint's encoded JSON body keyed on its query string plus the data version, used by `/stats/leaderboard`, `/stats/scatter` and `/stats/compare`; it stores bytes, so a hit cannot be modified by the request that reads it, and an error is never stored. ⚠️ Only for results with no user data in them, cached values are shared (callers that modify one copy it first), and it is **off under `ENVIRONMENT=test`**, because a rolled-back test transaction never moves the counters. |
...
| `seasons.py` | ⭐ **Which seasons exist, and which is current (M6.0).** "Current" = the newest season *with stats*, not the newest on the schedule — those differ for most of the year, and defaulting to the latter opens a board on an empty table. Shared by `/seasons` and the draft board. Also holds the **week** clock: `newest_played_week` (the newest week *mostly* final) and `next_unplayed_week` (the first week after it with a game to play), shared by the home scoreboard and the Vegas board's default week. ⚠️ And `week_bounds()` — the first and last week of a season *with stat lines*, which three surfaces used to ask separately and which is the most expensive cheap-looking query in the app: given accurate statistics Postgres walks the `week` index from both ends and discards other seasons, ~110 MB of random reads for a season in week 1. Cached per data version, and indexed for the cold read (migration `1dbc965aa956`) — a cache alone would leave the trap live for the first request after every write. |
| `vegas.py` | ⭐ **The Vegas board (M6.4).** Splits each game's spread and total into **implied team totals** — the points the market expects an offense to score, and the sharpest forward-looking read on how many fantasy points will exist. No odds API and no new columns: the lines arrive in the M6.0 schedule ingest, and implied totals are derived rather than stored. Handles *unpriced* as a first-class state — the market prices a few weeks out, so most of a season carries no line in August, and a null must never sort as a low total. Opens on the scoreboard's "next" week (`app/seasons.py`), so a Monday shows the coming week's lines rather than Monday night's one. |
...
| `intelligence.py` | ⭐ **The fantasy-intelligence engine (M3).** VORP, Fantasy Opportunity Rating, Positive-Regression (buy-low) Index, and Sell-High Index — built from percentile ranks within each position pool, plus career-baseline efficiency and usage trend. Also computes **expected VORP** (M6.1) — the same replacement-level maths run on expected points instead of actual, so it values opportunity rather than the finish it produced — and resolves trailing windows and produces the per-input `breakdown()` the player page renders. All weights and thresholds are constants at the top of the file; see [`docs/design/M3-fantasy-intelligence.md`](docs/design/M3-fantasy-intelligence.md). Scored windows, week bounds and **career totals** are cached per data version (`cache.py`). The career baseline sums raw scoring components once and prices them per request with `compute_points`, so one read of every earlier season serves every league's scoring. |
...
| `schemas/stats.py` | Stat-line (game log) response shape. Carries every stored column a leaderboard tab can show (first downs, sacks, PFR charting, Next Gen, the first-down expected components), so the player page's game log can show any tab one game at a time. |
| `schemas/team.py` | Team response shape. |
| `schemas/game.py` | ⭐ **Game response shapes (M10).** `GameOut` carries the fixture, the result once it exists, and the market's view — including a resolved `favorite` / `favorite_spread`, so no client has to remember that a positive `spread_line` favours the *home* team. `ScoreboardOut` is the two-week home window, and each half names its own season because from January to September they straddle two. `ScoreboardWindow.records` maps team abbreviation to a **formatted** record ("2-0", "1-1-1") — a map rather than two fields on every game, because a record is a fact about the season and not about the fixture, and the tie is formatted once here rather than by every surface that shows one. |
...
| `routers/players.py` | `GET /players` (search/list, plus **`player_ids`** — M10, resolve a known list of players in one request rather than one call each; the leaderboard is the wrong endpoint for that, since it aggregates stat lines and a player with none in the window vanishes), `/players/{id}` (profile — carries the M6 bio columns, a computed age, and the current **depth-chart slot** with its date), `/players/{id}/stats` (game log — each line also carries its **weekly finish** among the position, `position_rank` / `pool_size`, from one `RANK()` window query in the request's scoring; M13), `/players/{id}/career` (**M13** — season by season with each season's position finish, scoring-aware; see `app/career.py`), `/players/{id}/intelligence` (M3 scores + breakdown), `/players/{id}/target-depth` (M4 depth buckets). |
...
| `test_engine_cache.py` | ⭐ **The engine cache.** A repeat call served without recomputing, a data write forcing a miss, LRU eviction, and the one-hour backstop; that `build_intelligence` hands out **copies** (the Insight endpoint writes percentiles onto its rows and sorts the list in place, and neither may reach the next request); and that the career baseline, now summed once and priced in Python, still prices a TE premium. Switches the cache on per test, since it is off under `ENVIRONMENT=test`. |
| `test_response_cache.py` | The response cache on `/stats/leaderboard`, `/stats/scatter` and `/stats/compare`: a repeat request (in any parameter order) served without recomputing, two watchlists never sharing an answer, a data write forcing a miss, a 404 never stored, and a hit sending the same bytes as the uncached endpoint. |
...
| `app/percentiles.py` | ⭐ **Percentile ranks for board columns (M12)**, and **positional ranks** from the same pools (`ranks_for_row`, M13 — what a player page's headline shows as "WR4"). A rank is competition-ranked (ties share, the next skips) and direction-corrected exactly like the percentile, and it is read from the identical qualified pool so the two can never describe different players. One pool per (position, season), mid-rank, built on `intelligence.Pool` rather than a second implementation. Direction comes from the registry's `higher_is_better` and is inverted *before* reporting, so 95th always reads as good even for drops. ⚠️ The pool is deliberately **unfiltered** by team, watchlist or the caller's `min_games` — narrowing a board must not change what a percentile means. |
| `app/career.py` | ⭐ **Career seasons with their position finishes (M13).** One query grouped by `(player_id, season)` covers a whole career, rather than N leaderboard-sized aggregations for a 23-season quarterback. Two finishes on two pools, deliberately: **total points ranks everyone who played** (a season cut short still produced what it produced), while **per game ranks only players who qualified**, on the same `QUALIFY_FRACTION` rule the Insight and percentile pools use — so a two-game cameo at 28 PPG cannot post the league's best season. Scoring-aware, so a superflex league re-ranks the career. |
...
| `alembic/versions/d4c8e1f07a3b_*.py` | **`teams.logo_url`.** One nullable column holding nflverse's ESPN logo URL (`team_logo_espn`, which `ingest_teams.py` used to discard), hotlinked by the home scoreboard the way headshots are. |
| `alembic/versions/1dbc965aa956_*.py` | **`player_stats (season, season_type, week)`.** 712 kB, and the whole point is the leading `season`: without it Postgres answers `MIN(week)/MAX(week)` for one season by walking the single-column `week` index across all of them — ~110 MB of random reads for a season one week old, against 6 pages with it. `ix_player_stats_season` is left alone; dropping it is a separate judgement. |
...
| `ingest_usage.py` | **Run 6th** (enrichment). Snap counts (PFR, joined via a `pfr_id → gsis_id` crosswalk) and route usage (participation × play-by-play), then derives TPRR/YPRR — for the snap seasons too, so hand-loaded routes stay in step with stat corrections. `--skip-routes` for snaps only. |
| `ingest_routes.py` | ⭐ **In-season routes, hand-loaded.** No free feed has routes during a season, so a weekly charting export goes in `data/routes/` and this attaches it to existing stat lines: `routes_run`, `route_participation` over **dropbacks including scrambles** (against pass plays 10 of 282 players read over 100%), then `derive_route_rates`. Matches on team + normalised name with a unique-surname fallback, logs every miss by kind, and **cross-checks** the export's snaps/targets/carries without writing them. `--dry-run` writes nothing. Production is loaded by running it locally against the production database — the file is never in CI. |
| `data/routes/` | **Gitignored apart from its README** — the exports are licensed and the repo is public. Files are named `<season>-w<week>[-label].xlsx` (or `.csv`); several may cover one week and apply in name order. |
...
| `ingest_expert_boards.py` | ⭐ **Expert CSV boards (M9).** Reads `data/rankings/*.csv` and blends them **anonymously** into the Second Level Consensus — several are paywalled, so the script refuses to write a source id the API publishes and never writes a display name. A single expert's rank goes in `ecr` with `sd`/`best`/`worst` left NULL: one person is not a consensus and a zero spread would claim perfect agreement. |
...
| Add or tune an **Insight score** (M3) | `backend/app/intelligence.py` (the weights/terms are constants at the top) + a `backend/app/metrics.py` entry with `aggregation="intelligence"`; expose in `frontend/src/constants/index.js` and add it to a board's columns in `constants/boards.js` or a preset in `constants/leaderboards.js`. No DB or pipeline work — these are query-time. |
...
| Change what a **leaderboard tab** shows for a position | edit that group's preset in `frontend/src/constants/leaderboards.js` (and its default sort in `DEFAULT_SORT`). Edit Columns picks up any stat added to a single-position preset automatically, via `groupPool()`. |
| Add a new **Insight board** | add a board in `frontend/src/constants/boards.js`. The route, nav dropdown item, and page pick it up automatically. |
...
| Change **data scope** (seasons, etc.) | re-run `pipeline/` scripts with new `--seasons`. The season *list* is no longer edited anywhere: the backend derives it from the data (`routers/seasons.py`) and the frontend reads it (`hooks/useSeasons.js`). Only `FIRST_SEASON` (in `pipeline/seasons.py`, mirrored in `backend/app/availability.py` and `constants/index.js`) is a decision rather than a fact — **2009** since September 2026, the first season every fantasy-first metric can be answered in (it was 1999 from M8, the floor of nflverse play-by-play, until migration `85d024666c8f` dropped those seasons). Widening it again means lowering all three and re-running the backfill. A *metric's* season window is a measured fact and lives in `availability.py` (both copies). |
...
- **Fantasy points** — a weighted score of a player's real stats (yards, TDs, etc.)
  under a league's scoring rules. Second Level computes these live from raw stats.
...
- **Replacement level** — the points-per-game of that last startable player. Depends on
  league size and starting lineup, which is why Second Level makes you set them.
...
- **2026-09-24**: **The player leaderboards rebuilt as one page.** The fourteen M12 boards
  (four areas crossed with four lenses) become `/leaderboards/:tab`: five **preset tabs**
  (Fantasy, Usage, Efficiency, Expected, Tracking), a **Custom** tab, and a **position
  group** control (QB, RB, WR, TE, WR/TE, RB/WR/TE, All; default RB/WR/TE) that stays put
  across tabs. Every stat has one home tab, FPPG leads every preset, and seven stats that
  sat on no board get one (FP per route run, FP per carry, fumbles, fumbles lost, drops,
  drop rate, passer rating when targeted); Next Gen's own CPOE, catch rate, intended air
  yards and air-yard share stay off. **Edit Columns** opens a slide-out that adds, removes,
  searches and drag-reorders, live. Every tab is served by `/stats/intelligence` with
  percentiles for **every stat in the position group**, so a column change sends no
  request (measured: three edits and a drag, zero requests). New
  `constants/leaderboards.js`, `components/leaderboard/` (`LeaderboardTabs`,
  `PositionGroupPicker`, `ColumnEditor`); `pages/LeaderboardView.jsx` rewritten;
  `constants/boards.js` keeps only Insight, Schedule and the hidden sections, and
  `NAV_GROUPS` gains a Leaderboards dropdown. `components/BoardTabs.jsx` and
  `components/ui/MegaDropdown.jsx` removed. The old paths redirect to the matching tab and
  group with their filters (`LegacyBoard` in `App.jsx`), so saved views survive; the home
  page's and player page's board links point at the new paths. Frontend only: no
  migration, no API change. Follow-ups the same day: Scoring moved into the filter row
  beside Team; the menu's tab descriptions rewritten; the Timeframe button names its weeks
  ("Week 2", "Weeks 1, 2") instead of counting them, and two bugs there fixed (Clear
  wrote `weeks=[object Object]` into the URL, and early in a season weeks 1-2 were
  labelled "Last 4"); and the Edit Columns panel rebuilt as board-plus-library, the
  option picked from four mockups.
- **2026-09-24**: **The player page takes the leaderboard's five tabs.** Season Stats' three
  boards (Fantasy / Production / Advanced) become five rows, one per leaderboard tab, from
  the same presets (`seasonBoards()`); the ten-stat headline and the plain row labels are
  unchanged. The career table and the game log each get their own tab bar, Custom tab and
  Edit Columns (`TableViewBar`, state in `hooks/useTableView.js`, in the URL as
  `?career=`/`?log=` plus `_cols`), defaulting to Fantasy and Usage, with their fixed lead
  blocks kept and those stats left out of their tabs. `CAREER_GROUPS`, `GAMELOG_GROUPS` and
  `SEASON_BOARDS` are gone. Backend: `StatLineOut` gains the stored columns the game log
  was missing (first downs, sacks, PFR charting, most of Next Gen, the first-down expected
  components); additive, no migration. `ColumnEditor` takes `exclude`, `boardTitle` and
  `subtitle`; `LeaderboardTabs` takes `onSelect` and `size`. Option 2 of three mockups.
  Same day: the career and game log get the player page's own Fantasy tab per position
  (QB: CMP, ATT, PASS YD, PASS TD, INT, CAR, RUSH YD, YPC, RUSH TD; RB: CAR, RUSH YD, YPC,
  RUSH TD, TGT, REC, REC YD, REC TD; WR and TE: TGT, REC, REC YD, Y/TGT, Y/REC, REC TD), and
  the career's lead block gains Snap% to match the game log's.
- **2026-09-22**: **Response cache on the three uncached home-page endpoints.** Production
  started returning 502s and 503s on data endpoints while `/health` stayed up, the Disk
  IO starvation pattern again. Measured locally with the app's engine caches warm, one
  home-page visit still touched about 14,000 Postgres pages (~110 MB), all of it from
  `/stats/scatter` (~27 MB), `/stats/compare` (~24 MB) and four `/stats/leaderboard`
  calls (8 to 21 MB each), none of which were cached. New `cached_response` in
  `app/cache.py` stores each endpoint's encoded JSON body keyed on its query string and
  the data version; the same visit now touches none of those pages once warm. Bodies are
  byte-identical to the uncached endpoint, checked against `main` for every home-page
  call. New `tests/test_response_cache.py` (8 tests); 387 backend tests pass.

- **2026-09-22** — **Home page polish.** Four fixes. `GET /games/scoreboard` now returns
  a `records` map per window (`_season_records()` in `routers/games.py`) and the scoreboard
  shows each team's record beside its abbreviation. Expected vs Actual gets plainer copy and
  draws its par line in a new `--plot-rule` token — ⚠️ it used `--border-strong`, which is a
  glass *edge* and near-white on the light theme, so the line was invisible there; measured
  3.51:1 on dark and 10.80:1 on light now. The featured head-to-head becomes Jalen Coker
  against Tetairoa McMillan, two receivers on one offence, with 96px headshots and a
  **route-shaped axis set** (PPG, expected, targets, RTE%, red zone targets, ADOT, YPRR,
  TPRR) in place of the rushing axes the old committee pairing needed — the axes follow
  the matchup, not the other way round. Trending Players' headshots go to 56px, and its
  identity column to 215px so the longest name still clears the ellipsis. And
  `Layout.contentWidth()` becomes the single owner of the content column's width, read by
  both `<main>` and the credit bar, which fixes a footer that stayed at 1,280px after the
  home page moved to 1,560px. Verified aligned on `/`, `/teams`, `/fantasy/all`,
  `/insight/sos` and `/schedule/games`. 379 backend tests pass.

- **2026-09-22** — **The home page rearranged, and its arrangement made data.** The page
  ran 3,893px with 3,515px of that in one column and 445px in the rail. Every card's height
  was measured at nine column widths, four arrangements were built behind a `?layout=`
  switcher, and the two-column one with the boards moved into the rail was chosen: container
  1,560px, columns 983/561, **2,855px (27% shorter)**, and the rail carries real weight
  (1,591px) instead of one card. The others measured 1,970 (three columns), 2,667 (a
  full-width Trending Players over two) and 2,139 (the same hero over three); all are in git
  history. `constants/homeLayout.js` holds the result, `pages/Home.jsx` builds a keyed
  `cards` object and renders it, and `<main>` drops its 1280px cap on `/` so the page can own
  its width. Stacking to one column below 1,200px was tried and dropped: it avoids at most
  78px of sideways scroll on one card and costs ~2,100px of page height.

- **2026-09-22** — **Trending Players, and one plot for expected vs actual.** The Week
  Standouts card becomes `components/home/TrendingPlayersCard.jsx`: four stats per pick,
  **chosen per pick** rather than by position, with the change stated as an arrow and a
  delta instead of drawn as a dumbbell. A pick names its own basis (`TRENDING_BASIS`) —
  the same player last week, a teammate the same week, or no change at all and the
  season's positional ranks — so a back who took a job and a quarterback who has simply
  been good can share a card without either being misdescribed. The two rail signal cards
  are replaced by `components/home/ExpectedActualCard.jsx` in the wide column: the league
  as a faint cloud from `/stats/scatter`, par as the diagonal, the picks as headshots on
  collision-avoiding leaders, and the cause of each gap in a `StatTooltip` derived from
  the stored expected components. New `formatDelta()` in `utils/format.js` (a share moves
  in **percentage points**, not percent). No backend, migration or pipeline change: every
  number already had an endpoint. ⚠️ The sticky rail now holds only the scoreboard.

- **2026-09-21**: **The rebrand reaches the server and the docs.** The September 20 pass renamed
  every user-visible string in the frontend; this one covers what it did not reach. User-facing:
  the FastAPI app's title and its root `service` field (both render on the public `/docs` page),
  and `app/rankings.py`'s blend label, which is the board name the Draft surface shows once that
  section is un-hidden. Developer text: the READMEs, `render.yaml`, `docker-compose.yml`,
  `.env.example`, the workflow comments and every design doc. The standalone Word write-up is
  renamed to `Second-Level-Technical-Walkthrough.docx`, its own five mentions included.
  ⚠️ **The infrastructure identifiers are deliberately unchanged**: the Render service
  `gridiron-api`, the Postgres role/database `gridiron`, `gridiron_test`, and the `gridiron.*`
  localStorage keys. Each is a name two systems have agreed on, so renaming one is a coordinated
  migration rather than a rename, and none of them is visible to a reader. The two
  `docs/GridironIQ-*.xlsx` files keep their names for now because they had uncommitted edits.
  ⚠️ `config.py`'s `cors_origin_regex` still hardcodes the `gridiron-` Vercel prefix, which is
  the one rename that has to happen **with** the domain change rather than before it.

- **2026-09-20**: **The Second Level rebrand, wired in.** `components/Brand.jsx` replaces the
  placeholder arc-and-dot `BrandMark` that lived inside `Layout.jsx`; the header lockup is now a
  link to `/`. New `public/favicon.svg`, `apple-touch-icon.png` and `icon-512.png`, and the page
  title becomes "Second Level | Fantasy Football Data". Every user-visible "GridironIQ" string is
  renamed (17 across 16 files), including the **CSV download filenames**, which were `gridironiq-*`
  and are now `second-level-*`. ⚠️ **The `gridiron.*` localStorage keys are deliberately unchanged**
  (`gridiron.theme`, `gridiron.scoring`, `gridiron.mock`, `gridiron.styleguide.draft`): renaming a
  storage key does not migrate it, it silently resets every existing visitor's theme, scoring and
  in-progress mock draft. The full vector identity lives in `brand/second-level/`.
  The header lockup is **stacked** (mark over wordmark) at the artwork's own ratios; the wordmark's
  cap is floored at 9px because the canonical ratio puts it near 5px at header scale, and the
  tracking is solved from the target width so the lockup still spans what the artwork specifies.
  ⚠️ **The credit bar is no longer fixed.** It sits in normal flow at the end of the page, which
  removed both the reserved-height coupling in `Layout.jsx` and the measured-copy-length budget,
  and is what made room for the `SlashLockup` in it. It is a **`glass-card` constrained to the
  content width**, not a full-bleed band, so its edges line up with the boards above it. The
  bespoke `.glass-footer` rule is deleted.
...
- **2026-09-17**: **Four small surface fixes.** The week rail dropped its state words
  ...
  already said it; an unpriced game card dropped its "no line yet" caption the same way,
  leaving the empty bar to say it. `SLOT_ORDER` gained **Wednesday Night** at the top, so the two 2026
  Wednesday games head their slate instead of landing in "Other" below Monday. The
  player page's game log now covers the **whole season**: `buildSeasonLog()` in
  `GameLog.jsx` merges his stat lines onto his team's fixtures, so weeks still to come
  are rows of dashes and the bye is a greyed row. And **My Players** on the Command
  Center shows each value's percentile at that position beneath it, through the
  `percentiles=` param the boards already use.
...
- **2026-09-15** — **Command Center: the scoring pill, and My Players rebuilt.** The
  ...
  re-prices every number on it. It is now `components/ScoringPill.jsx`, writing through
  `useScoring` (default still out of the URL, non-default still in it, still mirrored to
  `localStorage`). **My Players** gained the seven columns that make it a board rather
  than a glance — FPTS, FPPG, FOR, OPP%, TGT%, RTE%, RUSH% — and that moved it off
  `/stats/leaderboard`: **FOR has no stored column**, so the card is served by
  `/stats/intelligence`, where the watchlist is an output filter applied *after*
  scoring and a starred player's percentile still means "against his whole position".
  `include_unqualified` is passed so a starred player who missed games stays on his own
  card. Seven stat columns do not fit the 300px sticky rail, so the card moved to the
  wide column beneath Week standouts — the one deviation from M10's "watchlist is rail
  reference" layout, and an easy one to undo.

- **2026-09-15** — **Schedule tab redesign** (directions A and F+E from the mockups).
  **Games** is a slate of cards grouped by kickoff slot rather than a flat table: the
  old board hid the slot (a reader parses a week as Thursday / Sunday early / Sunday
  late / Sunday night / Monday and had to reconstruct that from timestamps), buried the
  implied totals in a shared "Implied A/H" cell, and gave a 52.5-point shootout the same
  visual weight as a 40.5-point slog. The week `<select>` became a **rail** carrying
  played/priced state per week, and the page opens on the first unfinished week instead
  of all 272 fixtures — resolved client-side and kept out of the URL, so a shared link
  carries a week only when the sender picked one. Across weeks it groups by week, which
  is what makes `?week=all&team=12` a readable single-team season. **Vegas** lost its
  Players/Games toggle for one scroll — `TeamEnvironmentRail` (every offence, one bar,
  ranked by implied total) over `OffenseCard`s (the same offences with their players as
  chips). Losing the toggle *was* the fix: the players view ranked ~300 rows on 32
  distinct values, so it opened with twelve San Francisco players in a row. New
  `components/schedule/`; no API change, since `/games` already carried logos and
  implied totals. Two colour bugs caught and now recorded as project rules: Tailwind's
  `/opacity` modifier renders **transparent** on this project's `var(--token)` colours
  (half of every split bar was invisible), and a solid `--accent` fill measures 2.83:1
  against white text in the light theme.

- **2026-09-15** — **Launch trim.** Four cuts and one fix, all frontend; no migration, no
  API change. **Custom scoring is gone**: the picker offers only PPR / Half / Standard /
  TE-Premium, and `parseScoring` now *normalises* rather than parses, so a
  `ppr:te_rec=1.5` left in an old link or in `localStorage` resolves to its bare preset
  instead of showing "PPR" over a table priced as something else. **League profiles went
  with it** (`LeagueProfileBar`, `useProfileSync`, `useLeagueProfiles`, the account
  menu's profile list) — a "profile" whose whole contents is one of four presets is a
  name for something that already fits in a dropdown; the `/me/league-profiles`
  endpoints still stand, uncalled, so reinstating them is a frontend change alone.
  **VORP is retired everywhere** — the `/insight/vorp` board, the four `vorp*` columns on
  every fantasy board, the player page's Value group and Insight tile, and the two
  scatter presets built on `vorp_ppg` (dropped rather than re-axised: the builder is
  curated, and picking a replacement y-axis to keep the slot filled is the arbitrary
  pairing the curation exists to prevent). That left `LeagueControl` driving nothing
  visible — replacement level was its only consumer — so the league-size and
  starting-lineup editor went too, and `useLeague` is now a constant. **Draft and Explore
  are hidden**: removed from the nav *and* redirected to `/` via `HIDDEN_SECTIONS`,
  because an un-linked section still has bookmarks and search results. Bundle: 1,217 kB →
  684 kB, since those pages are no longer imported. Finally, **the nav dropdowns were
  finicky and now are not** — new `hooks/useHoverMenu.js`: the menus closed on the same
  frame the pointer left the trigger, and sat 6px below it across a gap belonging to
  neither element, so the natural diagonal toward an item lost the menu twice over. A
  ~260ms cancellable grace period, the offset moved from margin to padding on a
  hoverable wrapper, and a registry so an opening menu closes its siblings at once.

- **2026-09-15** — **Scope narrowed to 2009, and the week-bounds trap.** Migration
  `85d024666c8f` deletes the 1999–2008 seasons (52,146 stat lines, 2,646 games) — the M8
  audit's own finding, that those seasons carry a box score and little else, applied:
  targets are unrecoverable 2003–2008 and expected points have no usable receiving side
  until 2009, so a third of the stat lines could not answer a fantasy question.
  `FIRST_SEASON` moves in all three places (`pipeline/seasons.py`,
  `backend/app/availability.py`, `frontend/src/constants/index.js`); the per-metric
  windows are **kept** so widening scope again stays safe. `players` and `teams` are
  untouched (`load_players` republishes every retired player daily, so deleting them
  undoes itself). With `VACUUM FULL` afterwards — the delete frees pages but does not
  return them — the local database went 238 MB → 121 MB and `player_stats` 140 → 44 MB.
  Also: `MIN(week)/MAX(week) WHERE season = …` is now `week_bounds()` in
  `app/seasons.py`, cached, instead of three copies (Insight windows, the trending card,
  the percentile denominator); it read ~110 MB per call for a season in week 1. The
  trending card fell from 117 MB to 0.1 MB on a repeat, and to 45 MB cold once migration
  `1dbc965aa956` added the `(season, season_type, week)` index — the cache alone would
  have left that read live after every pipeline write.
- **2026-09-15** — **Engine cache, after Supabase's Disk IO budget warning.** Measured per
  request on a full local copy: `/stats/intelligence` read ~165 MB of pages and draft
  rankings ~206 MB, almost all of it `fetch_career_efficiency` scanning every earlier
  season of `player_stats` for every request and every scoring config. New `app/cache.py`
  (a bounded LRU keyed on a data version read from `pg_stat_user_tables`) now fronts
  scored windows, week bounds and career totals in `app/intelligence.py`, plus
  `season_summary` and `team_summary`. The career baseline sums raw components once and is
  priced per request, bit-for-bit identical to the old SQL in five scoring configs. Repeat
  requests: Insight board 165 → 0.1 MB, draft rankings 206 → 21 MB, team page 26 → 3 MB; a
  new scoring or league on a warm season 165 → 18 MB. Restricting the baseline to the
  players being scored was tried and abandoned: their older lines sit on ~60% of the
  table's pages. New `tests/test_engine_cache.py` (11 tests).
- **2026-09-15** — **Docs brought current after #25 shipped.** ROADMAP gains the M13 and
  2026-rollover sections and a decision-log entry for hand-loaded routes; its data table
  and `docs/design/M8-historical-depth.md` no longer call the participation feed ended; the
  M10 design note records the retired outlook mode and the scoreboard redraw; CLAUDE.md's
  status list and test count (367) are current. Production routes for 2026 Week 1 were
  loaded from a local machine (282 stat lines). Deleted `frontend/ARCHITECTURE.md`, a
  stale copy of this file last updated at M6.4 that nothing linked to.
- **2026-09-15** — **Command Center, Week 1.** New `components/home/StandoutsCard.jsx` takes the
  top slot: five hand-picked players (`WEEKLY_STANDOUTS`) and one week of usage with each
  value's positional rank as a `FinishChip`. `TrendingCard` is live-only now — its preseason
  outlook mode and `OPPORTUNITY_OUTLOOK` were removed. The scoreboard rows are one line per
  game with the date above the score or kickoff and team logos, from new column
  `teams.logo_url` (migration `d4c8e1f07a3b`, filled by `ingest_teams.py`, served on every
  game as `home_logo_url` / `away_logo_url`). The head-to-head table is now
  `components/player/MarginTable.jsx`, shared with the player page. Last Week's Scoring gains
  a FLEX tab and touchdowns in its stat line; the signal cards and head-to-head carry 2026
  Week 1 picks.
- **2026-09-14** — **In-season routes + the Vegas default week.** New `pipeline/ingest_routes.py`
  loads routes run from a hand-supplied weekly charting export in `pipeline/data/routes/`
  (gitignored — licensed data, public repo; production is loaded locally), writing
  `routes_run` and `route_participation` over dropbacks and re-deriving TPRR/YPRR; the
  registry's routes-per-game and FP/RR follow at query time. `ingest_usage.py`'s
  `derive_route_rates` is now public and runs for snap seasons too. `openpyxl` joins the
  pipeline requirements. The week clock moved into `app/seasons.py`
  (`newest_played_week` / `next_unplayed_week`), and the Vegas board opens on the same
  "next" week as the scoreboard.
...
- **2026-09-12** — **M13: position-aware finishes + new radar slices.** `FinishChip` now tiers by
  how deep the league starts each position (mirroring `replacement_ranks` without the bench)
  instead of one league-sized tier for everyone, so QB22 and TE22 read as unstartable. The
  season radar's slices were redrawn per position, with one new registry composite,
  `fantasy_points_per_carry`. The player header's dated depth-chart badge was removed.
- **2026-09-12** — **M13: finish chips.** New `components/player/FinishChip.jsx` colours every
  positional finish on the career table and the game log by league-sized tiers, and the game
  log's Finish column moved next to Pts. Frontend only.
- **2026-09-12** — **M13: game log by position.** The game log gets its own column lists
  (`GAMELOG_GROUPS` in `constants/playerPage.js`, split from the career table's) with a new
  Advanced group per position, and a **weekly finish** column: `/players/{id}/stats` now
  returns `position_rank` / `pool_size` on every line, computed by `_weekly_finishes` in
  `routers/players.py` as one window query in the request's scoring. `StatLineOut` gains the
  six stored columns the new groups read (contact yards, RYOE, bad-throw rate, receiver
  drops, intended air yards). New test `backend/tests/test_weekly_finish.py` pins ties,
  the position-only pool, and re-ranking under a different scoring. Also corrected a wrong
  M13 claim: Y/REC, Y/TGT and YPRR are stored per game, so they are no longer re-derived.
  **No migration.**

- **2026-09-12** — **M13: season headline + board rows.** The season-stats card becomes a
  ten-stat headline with positional ranks over the three boards as stacked rows sharing one
  scrollbar (`components/player/SeasonStatGrid.jsx`; the per-position lists are
  `HEADLINE_STATS` in `constants/playerPage.js`). Backend: `/stats/intelligence` gains
  `ranks=`, served by a new `PercentileIndex.ranks_for_row` in `app/percentiles.py` from the
  pools percentiles already use, and one new registry composite,
  `fantasy_points_per_route_run` (`fantasy_points/routes_run`, so it follows the league's
  scoring and exists wherever routes do). New pure unit test
  `backend/tests/test_positional_ranks.py` pins rank direction, ties and pool
  qualification. **No migration.** Also: the percentile panel's bullet lost its quartile
  bands, and the game log moved directly under the career table.

- **2026-09-07** — **M13: the player page rebuilt.** One new backend module (`app/career.py`)
  and one new endpoint (`GET /players/{id}/career`) — season by season with each season's
  finish among the position, re-ranked in the caller's scoring, on two deliberately
  different pools (total points ranks everyone who played; per game ranks only players who
  qualified). **No migration and no new stored column**: everything else the page needs was
  already reachable, and the season row now comes from `/stats/intelligence` because that is
  the only endpoint carrying both the stored columns and the query-time ones (VORP) *plus*
  percentiles — which also means a comparison's two players share one request and therefore
  identical pools. Frontend: a new `constants/playerPage.js` (every position-specific column
  list on the page, in one place) and `components/player/*` — career table, season stat grid,
  season radar, percentile ladder, head-to-head, compare radar, compare picker, game log.
  `StatTooltip` gained an overridable `hint`, since nothing on this page sorts. ⚠️ The rebuild
  **orphaned** `components/InsightPanel.jsx`, `components/charts/{FantasyTrendChart,
  UsageTrendChart,TargetDepthChart}.jsx` and the `SUMMARY_STATS` / `GAMELOG_COLUMN_SETS`
  constants — the old page was their only caller. They are left in place, not deleted:
  re-homing the M4 usage and target-depth charts is a decision, not a cleanup.
...
- **2026-09-06** — **Migration automation attempted and reverted.** `alembic upgrade
  ...
  replacing. The failure also exposed a verification error worth remembering: the
  deploy was reported here as succeeding because `/health` returned 200 and the metric
  count was right, but that was the *previous* build still serving, which is exactly
  what a failed build is supposed to leave running. **Checking that a deploy worked
  means checking the deploy, not the endpoint.** `render.yaml` now records what was
...
- **2026-08-30** — **M10: games, trending usage, the Schedule tab, and the Command
  ...
  `constants/signals.js` — hand-picked cards with a stated expiry. Full design note:
  [`docs/design/M10-command-center-schedule.md`](docs/design/M10-command-center-schedule.md).
  The Trending card
